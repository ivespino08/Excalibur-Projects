...
class EventType(Enum):
    ...
    TREE_NODE_SELECTED = auto()  # node selected by UCB
    TREE_NODE_EXPANDED = auto()  # new child nodes added
    TREE_NODE_PRUNED = auto()  # branch pruned due to high TDI
    TREE_BACKPROPAGATE = auto()  # promise scores updated
    TREE_PIVOT_SPAWNED = auto()  # new pivot subtree created

    # TDA events
    TDI_COMPUTED = auto()  # TDI score computed for a node
    MODE_SELECTED = auto()  # reconnaissance/exploitation/llm_decide

    # Memory events
    ENTITY_DISCOVERED = auto()  # host/service/credential/vulnerability found
    CONTEXT_COMPRESSED = auto()  # context compression triggered
    CREDENTIAL_PROPAGATED = auto()  # credentials propagated to pruned branches
